...
This document defines **conductor tests** in `tests/sweetests/`: tests whose
assertions require direct zome calls or substrate inspection on a live
SweetConductor. Standalone async tests can also exercise client/runtime
contracts; they are not conductor-boundary tests merely because they omit the DSL.
...
> is suitable for a dance test or a standalone runtime integration test. If the assertion
> requires direct evidence of Integrity execution or substrate records, use a conductor test.

Do not add a `DanceTestStep` solely to expose internal substrate evidence; see
...
Installs the production DNA. Use this default setup for assertions about the
unaugmented production surface. Probe-enabled tests separately assert Integrity
behavior while proving that augmentation preserves production DNA identity.
...
`setup_probe_enabled_conductor()` performs the following sequence. Steps 2 and 7
are assertions inside the helper.
...
The surface integration test separately calls `assert_probe_zome_dispatchable`
after setup. The helper does not itself make a production extern call; preserved
WASM identity is not a substitute for an executed production behavior assertion.
...
This document owns conductor-test isolation and evidence requirements. The
packaged coordinator policy remains owned by the Coordinator Surface Policy.
Concrete helper names and build ordering describe the repository harness;
changes to those helpers must preserve the stated isolation guarantees.

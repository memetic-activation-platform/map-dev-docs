...
2. **Tryorama tests** — Holochain's JavaScript-based
   [Tryorama](https://github.com/holochain/tryorama) framework. Existing TypeScript test sources live under `tests/tryorama/`;
   their current execution and coverage are not established here; see [Tryorama Tests](#tryorama-tests) below.

---

## Choosing Where a Test Belongs

Sweettests use the dance DSL and standalone async integration tests. The deciding question is *what layer the assertion is about*, not what feature
is being tested.
...
Choose the assertion boundary first. Dance tests can assert client-visible guest outcomes,
including projected validation findings. Proving which Integrity callback or substrate action
produced an outcome requires direct conductor-level evidence. Standalone runtime integration
tests, such as `commit_conflict_tests.rs` and `bootstrap_performance_tests.rs`, may also exercise
client operations without the fixture DSL; function shape alone does not determine the boundary.
...
and begins transactions through the real command path —
`Runtime::execute_command(SpaceCommand::BeginTransaction)`. It first provisions Core Schema
in an internal bootstrap transaction, checks load completion and descriptor completion, archives
that transaction, and verifies space-ID injection. Only then does it open the ordinary transaction
used for fixture execution. These checks do not establish every durable-readiness requirement
in the [bootstrap specification](core-schema-bootstrap/core-schema-bootstrap-design-spec.md).
...
pub enum DanceTestStep {
    ...
    Commit {
        // Illustrative subset; see test_steps.rs for the complete variant.
...
`tests/tryorama/` contains configuration and TypeScript test sources, including
`src/map_holons/holons/test-case-1.test.ts`, plus older material under `unused/`.
Their presence does not establish current execution or maintained coverage.
The root `npm test` script runs unit tests and Sweettests; it does not run this
Tryorama suite. Verify the suite separately before relying on its coverage.

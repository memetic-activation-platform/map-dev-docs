...
The durable readiness requirements below are normative design requirements;
their presence does not assert that every check is implemented. See the
[implementation alignment notes](core-schema-bootstrap-implementation-plan.md)
for the audited implementation gaps.
...
This section describes intended future provisioning. Current runtime and wire
contracts support a single space; they do not provide cross-space resolution
or transport. Under the speculative mirror design, an externally owned holon
would be represented by a local mirror in the receiving space. Mirror
resolution and cloning remain unspecified. A foreign local ID must not be
interpreted as a receiving-space identity.
...
The following require separate designs and are outside the initial bootstrap
lifecycle:

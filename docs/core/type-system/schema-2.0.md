...
Semantic inheritance applies only to populated properties and relationships whose `PropertyType` or relationship descriptor explicitly declares:

    InheritanceMode Additive

These inherited values contribute to the subtype's effective semantics, but they do not become locally populated state on the subtype descriptor.

Unless a property or relationship type explicitly declares additive inheritance, its populated values remain local.
...
### 4.2 Non-inherited singular relationship
...
Every descriptor described by `MetaHolonType` must locally populate exactly one `UsesKeyRule` relationship.

A key rule selected by an ancestor descriptor does not satisfy this requirement.
...
2. Otherwise, if the effective property descriptor defines a `DefaultValue`, copy the default into the property map.
3. Otherwise, report a missing-required-property violation.
4. Validate the completed property map.
...
Only the non-default case needs to be written explicitly:
...
- It provides the hierarchy over which explicitly additive semantic values are resolved.
...
2. `Extends` determines:
   ...
   - the hierarchy over which explicitly additive semantic values are resolved.
...
9. Populated values remain local unless their property or relationship descriptor declares `InheritanceMode Additive`. `InheritanceMode` is required, defaults to `None`, and is materialized during descriptor creation.
10. A default may be defined only for a required property. Defaults are materialized into the property map during creation and are not resolved as read-time fallbacks.

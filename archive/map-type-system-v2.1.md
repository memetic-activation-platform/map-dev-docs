...
Effective key-rule resolution uses descriptor lineage:

- For descriptor holons, prefer the applicable `Extends` lineage from most specific to most general.
- Use the first `UsesKeyRule` found in that `Extends` chain.
- If no key rule is found through `Extends`, fall back through `DescribedBy` / `type` lineage.
...
`NoneRule.KeyRuleType` marks a type as keyless.

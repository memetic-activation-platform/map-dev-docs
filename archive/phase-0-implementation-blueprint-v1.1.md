...
- Phase 0 consumes effective flattened descriptor semantics from core rather than reconstructing them in TS
...
export interface RelationshipDescriptorHandle {
  ...
  minCardinality(): Promise<number | null>;
  maxCardinality(): Promise<number | null>;
  isDirectlyMutable(): Promise<boolean>;
}
...
export interface CommandDescriptorHandle {
  reference(): HolonReference;
  name(): Promise<string>;
  label(): Promise<string | null>;
  description(): Promise<string | null>;
...
export interface HolonTypeDescriptorHandle {
  ...
  commandDescriptors(): Promise<CommandDescriptorHandle[]>;
}
...
- keep dances and commands descriptor-driven
...
- represent dances and commands as a renderable affordance hierarchy
...
- build a minimal `AffordanceNode[]` tree from dance and command descriptors
...
- affordance visualizer choice is also trivial and happens inside the generic node visualizer composition
...
- render dances and commands through the default affordance visualizer
...
5. for each property:
   ...
   - surface requiredness and validation-relevant hints from the descriptor layer
...
- adapt `AffordanceNode[]` for the node visualizer’s affordance section
...
- actual dance/command invocation wiring may be stubbed if not yet ready
...
- dances and commands are visible as actions
...
- affordance section integration
...
- affordance hierarchy builder maps dances and commands into `AffordanceNode[]`
...
- `ActionMenuVisualizer` renders affordance groups/items
...
3. What public SDK shape exposes descriptor affordances such as dances and commands?
4. Is actual dance/command invocation in Phase 0 required, or is discovery/presentation sufficient?
...
3. show:
   ...
   - descriptor-defined dances and commands in `ActionMenuVisualizer`

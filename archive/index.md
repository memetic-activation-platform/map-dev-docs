...
## Shared Operand Contract Vocabulary

### Role in Dance Refactor

Defines the narrow shared operand family used by query-adjacent contracts and serialized payloads:

- `Value`
- `Row`
- `RowSet`

This document gives the refactor a stable shape vocabulary before broader algebra, planner, or descriptor semantics are layered on.

### Implementation Implications

During Dance refactor:

- use `Value`, `Row`, and `RowSet` as contract/result vocabulary
- keep their scope narrow and shape-oriented
- do not treat this document as the semantic source of operator or descriptor meaning
- do not assume internal execution must eagerly materialize all intermediate bindings as rows

This stabilizes payload language without constraining the internal binding model.

---

# 2. execution-type-resolution.md
...
# 5. cypher-operator-inventory.md
...
```
shared-operand-family-foundation.md
        ↓
...
2. Introduce shared operand contract vocabulary:
   - `Value`
   - `Row`
   - `RowSet`
3. Introduce `ResolvedType`.
